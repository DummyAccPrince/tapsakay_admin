...
const defaultDashboardStatsResponse: DashboardStatsResponse = {
  stats: {
    totalUsers: 0,
    totalDrivers: 0,
    totalBuses: 0,
...
export const useDashboardStats = () => {
  const { data, pending, error, refresh } = useFetch<DashboardStatsResponse>('/api/dashboard/stats', {
    key: 'dashboard-stats',
    default: () => defaultDashboardStatsResponse
  })

  const safeData = computed(() => data.value || defaultDashboardStatsResponse)

  const stats = computed(() => safeData.value.stats)
  const trends = computed(() => safeData.value.trends)
  const activeTrips = computed(() => safeData.value.activeTrips)
  const performanceSeries = computed(() => safeData.value.performanceSeries)
  const fleetIntelligence = computed(() => safeData.value.fleetIntelligence)
